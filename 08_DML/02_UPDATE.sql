...
-- DEPT_COPY 테이블에서 DEPT_ID가 D9인 부서명을 기획팀으로 수정
UPDATE DEPT_COPY
SET DEPT_TITLE = '기획팀'
WHERE DEPT_ID = 'D9';

-- EMP_SALARY 테이블에서 노옹철 사원의 급여를 1000000원 으로 변경
UPDATE EMP_SALARY
SET SALARY = '1000000'
WHERE EMP_NAME = '노옹철';
...
-- EMP_SALARY 테이블에서 선동일 사원의 급여를 7000000원, 보너스를 0.2로 변경
UPDATE EMP_SALARY
SET SALARY = '7000000', 
       BONUS = 0.2
WHERE EMP_NAME = '선동일';
...
-- EMPLOYEE 테이블에서 사번이 200번인 사원의 사번을 NULL로 변경
UPDATE EMPLOYEE
SET EMP_ID = NULL   -- NOT NULL 제약조건에 위배된다
WHERE EMP_ID = 200;

-- EMPLOYEE 테이블에서 사번이 200번인 사원의 사번을 210로 변경
UPDATE EMPLOYEE
